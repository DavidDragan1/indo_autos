<h2 class="breadcumbTitle">Inspection</h2>
<!-- cars-area start -->
<div class="card-wrap-style-two faq-table-wrap">
    <h3 class="card-header-withbtn">Inspection List <button data-toggle="modal"
                                                         data-target="#mechanicDiagnosticQuestionsType" class="header-btn">+ Add New</button>
    </h3>
    <div class="row">
        <div class="col-lg-4 offset-lg-8">
            <form class="search-form" action="admin/diagnostics/inspection" method="get">
        ...
        <table>
            ...
            foreach ($questions as $question) : ?>
                <tr>
                    ...
                    <td><?php echo $question->inspection; ?></td>
                        ...
                        <ul class="actionsbtn">
                            <li>
                                <button onclick="update_period(this)" data-id="<?php echo $question->id; ?>"
                                        data-problem="<?php echo $question->inspection; ?>"
                                        data-description="<?php echo $question->description; ?>"
                                        data-question="<?php echo $question->problem_id; ?>"
                                        data-status="<?php echo $question->status; ?>"
                                ><i class="fa fa-pencil-square-o"></i></button>
                            </li>
                            <li><a href="admin/diagnostics/delete-inspection/<?php echo $question->id; ?>"
...
<div class="modal fade modalWrapper" id="mechanicDiagnosticQuestionsType" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form class="row" method="post" action="admin/diagnostics/inspection-update">
                <div class="col-12 text-center">
                    <h3 class="title-add-car">Update Inspection</h3>
                </div>
                <div class="col-12">
                    <label class="input-label">Select Problem</label>
                    <div class="select2-wrapper">
                        <select class="input-style" name="problem" id="problem">
                            <?php echo makeProblem(getLoginUserData('user_id')); ?>
                        </select>
                    </div>
                </div>
                <div class="col-12">
                    <label class="input-label">Inspection Title</label>
                    <input name="inspection" id="inspection" type="text" class="inputbox-style" placeholder="Type Problem Title">
                </div>
...
<script>
    function update_period(element) {
        ...
        $("#inspection").val($(element).data("problem"));
        CKEDITOR.instances['problemDescription'].setData($(element).data("description"))
        $("#problem").val($(element).data("question")).change();
