...
function getBrowserList( $selected = 0 ) {
        $ci =& get_instance();
        $browsers =  $ci->db->select('browser, COUNT(id) as total')->group_by('browser')->get('login_history')->result();
...
function getDeviceList( $selected = 0 ) {
        $ci =& get_instance();
	$devices = $ci->db->select('device, COUNT(id) as total')->group_by('device')->get('login_history')->result();
