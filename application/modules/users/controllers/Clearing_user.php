...
class Clearing_user extends Admin_controller
{
    ...
    public function index($id){
        ...
        $config['per_page'] = 10;
        $config['base_url'] = Backend_URL . 'users/clearing-agent-product/'.$id;
        $config['first_url'] = Backend_URL . 'users/clearing-agent-product/'.$id;
        $this->db->from('clearing_product')->where(['user_id' => $id]);
        ...
        $this->viewAdminContent('users/clearing/index', $data);
    }

    public function product_create(){
        $data = [
            'action' => 'admin/clearing/product_create_action',
            'vehicle_type_id' => 1,
            'title' => '',
            'shipping_port' => 0,
            'destination_port' => 0,
    ...
    public function update($id){
        $row = $this->db->get_where('clearing_product', ['id' => $id])->row();
        if (!empty($row)){
            $data = [
                'action' => 'admin/users/clearing-agent-product/create-action',
            ...
            $this->viewAdminContent('users/clearing/update', $data);
        } else {
            $this->session->set_flashdata(['status' => 'error', 'message' => 'No Data Found']);
            redirect(site_url( Backend_URL. 'clearing/product'));
        }
    ...
    function delete($id){
        $this->db->where('id', $id);
        $this->db->delete('clearing_product');
    ...
    function create_action (){
        ...
        $total_amount = $clearing_amount + $ground_logistics_amount + $vat_amount;
        ...
        if (!empty($id)){
            $this->db->where('id', $id);
            $this->db->update('clearing_product', $data);
            $msg = 'Product Updated';
        } else {
            $this->db->insert('clearing_product', $data);
            $msg = 'Product created';
        }

        $this->session->set_flashdata('message' , $msg);
        redirect(site_url( Backend_URL. 'users/clearing-agent-product/'.$data['user_id']));
    }
