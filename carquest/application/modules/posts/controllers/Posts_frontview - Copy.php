       ...
       if($this->input->get('device_code', TRUE)){           
        ...
        if( $this->input->is_ajax_request()  ){
           $json = [
                ...
                'count'     => $total,
                'sql'       => $query_string,
                'orderBy'   => GlobalHelper::getShortBy('', $page_slug )
            ];
        ...
        if (!$slug) {
            ...
            if($post) {
                ...
                $data = array(
                    ...
                    'towing_service_id'       => $post->towing_service_id,
                    'towing_type_of_service_id'       => $post->towing_type_of_service_id,
                    'availability'       => $post->availability,
                    'vehicle_type'       => $post->vehicle_type,
                    
                    'towing_service_name'       => GlobalHelper::get_towing_services($post->towing_service_id),
                    'towing_type_of_service_name'       => GlobalHelper::get_towing_type_of_services($post->towing_type_of_service_id),
                    'vehicle_type_name'       => GlobalHelper::get_DropDownVehicleTypeTowing($post->vehicle_type),
                ...
                $data['current_status'] = ($user['current_status'] == 'Online' && ( $user['last_access'] > time()) ) ? 'online' : 'offline';
                $data['last_status'] = ($user['current_status'] == 'Online' && ( $user['last_access'] > time()) ) ? 'Online' : 'Offline';
                ...
                if( $post->status != 'Active' && empty($user_id) ){
                    ...
                    if( $data['post_type'] == 'Towing' ){
                        $this->load->view('frontend/template/single_towing', $data);
                    }else {
       ...
       /* echo '<pre>';
        ...
        if( $current_type->vehicle_type_id == 4 ) {
            ...
            $related_posts = $this->db
                ->order_by('id', 'RANDOM')
                ->get_where('posts', [ 'vehicle_type_id' => 4,  'status' => 'Active', 'id <>' => $post_id ], 4, 0)
                ->result();
            ...
            $related_posts = $this->db
                ->order_by('id', 'RANDOM')
                ->get_where('posts', [ 'post_type' => 'Automech',  'status' => 'Active', 'id <>' => $post_id ], 4, 0)
                ->result();
            
        } else {
            $related_posts = $this->db
                ->order_by('id', 'RANDOM')
                ->get_where('posts', [ 'model_id' => $model_id, 'post_type' => 'General', 'status' => 'Active', 'id <>' => $post_id ], 4, 0)
                ->result();
        }
        ...
        if( $current_type->vehicle_type_id == 4 ) {
            $html .= '<h3>Similar Spare Parts</h3>';
        } else if( $current_type->post_type == 'Automech' ){
            $html .= '<h3>Similar Automech</h3>';
        } else {
        ...
        $html .= '<div class="clearfix"></div>';
        $html .= '<div class="col-sm-12 no-padding related-txt"><h4>People who search for this car also search for these products</h4></div>';
        $html .= '</div>';
         $html .= '</div>';
         $html .= '</div>';
...
//                } else {
   ...
   public function api_getPosts( $page_slug = null ) {        
    ...
    public function sidebar_text(){
        ...
        $html .= '<p><strong>Contact Online Mechanics to Resolve your care issues</strong></p>';
        $html .= '<p><strong>Looking to Join Our car Association to reduce hassle of problem when your car breakdown</strong></p>';
        $html .= '<p><strong>Looking to Hire a certified, policed checked and background checked drivers</strong></p>';
        $html .= '<p><strong>Mobile Mechanics and Mobile tyre fixers</strong></p>';        
        $html .= '</div>';
        $html .= '</div>';
